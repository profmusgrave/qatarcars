...
Other helper functions are included to easily switch between SI (International System) units (i.e. meters, grams, liters) and US customary units (i.e. feet, pounds, gallons)

- Meters ↔ feet for length, width, and height: `m_to_ft()` and `ft_to_m()`
- Kilograms ↔ pounds for mass: `kg_to_lbs()` and `lbs_to_kg()`
- Liters ↔ cubic feet for trunk volume: `l_to_cuft()` and `cuft_to_l()`
- Liters/100km ↔ miles per gallon for fuel economy: `lp100km_to_mpg()` and `mpg_to_lp100km()`
- 0–100 km/h ↔ 0–60 mph for performance: `kmh100_to_mph60()` and `mph60_to_kmh100()`
  ...
  - Since 60 mph ≈ 96.56 km/h, we scale the km/h time down by ≈96.56% to convert to the mph time
